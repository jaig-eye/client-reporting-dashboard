...
export default async function AdminPreviewPage({
  ...
  for (const r of currentMetrics) {
    ...
    if (ex) { ex.spend += r.spend; ex.clicks += r.clicks; ex.impressions += r.impressions; ex.conversions += r.conversions; ex.conversion_value += r.conversion_value }
    else dailyMap.set(r.date, { spend: r.spend, clicks: r.clicks, impressions: r.impressions, conversions: r.conversions, conversion_value: r.conversion_value })
  }
  ...
  const kwAggMap = new Map<string, AggKeyword>()
  for (const r of kwRaw) {
    const key = r.keyword_text.toLowerCase()
    const ex  = kwAggMap.get(key)
    if (ex) { ex.impressions += r.impressions; ex.clicks += r.clicks; ex.conversions += Number(r.conversions); ex.spend += Number(r.spend) }
    else kwAggMap.set(key, { text: r.keyword_text, impressions: r.impressions, clicks: r.clicks, conversions: Number(r.conversions), spend: Number(r.spend) })
  }
  const keywords      = Array.from(kwAggMap.values())
          ...
          <>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              ...
              <SparkMetricCard label="CTR" value={fmtPct(current.ctr)} delta={showCompare ? calcDelta(current.ctr, prior.ctr) : undefined} sparkData={ctrSpark} sparkColor="#3b82f6"
                benchmark={{ actual: current.ctr, target: effectiveBenchmarks.benchmark_ctr, actualLabel: fmtPct(current.ctr), targetLabel: fmtPct(effectiveBenchmarks.benchmark_ctr), color: '#3b82f6' }} delay={3} />
              <SparkMetricCard label="Conv. Rate" value={fmtPct(convRate)} delta={showCompare ? calcDelta(convRate, prior.clicks > 0 ? prior.conversions / prior.clicks : 0) : undefined} sparkData={crSpark} sparkColor="#10b981"
                benchmark={{ actual: convRate, target: effectiveBenchmarks.benchmark_conv_rate, actualLabel: fmtPct(convRate), targetLabel: fmtPct(effectiveBenchmarks.benchmark_conv_rate), color: '#10b981' }} delay={4} />
              <SparkMetricCard label="CPM" value={fmtCurrency(current.cpm)} delta={showCompare ? calcDelta(current.cpm, prior.cpm) : undefined} invertDelta sparkData={cpmSpark} sparkColor="#f59e0b"
                benchmark={{ actual: current.cpm, target: effectiveBenchmarks.benchmark_cpm, actualLabel: fmtCurrency(current.cpm), targetLabel: fmtCurrency(effectiveBenchmarks.benchmark_cpm), color: '#f59e0b' }} delay={5} />
            </div>
            ...
            {activeSource === 'google_ads' && keywords.length > 0 && (
              <div>
                ...
                <KeywordSummary keywords={keywords} negativeCount={negativeCount} conversionLabel={kwConvLabel} />
              </div>
            )}
