...
const getAdFuelBalance = unstable_cache(
  async (locationId: string) => {
    ...
    for (const e of (ledgerRes.data ?? []) as LedgerRow[]) {
      ...
      if (eMs >= CUTOFF_MS) afPurchased += Number(e.amount_af)
    }

    return Number((afPurchased - afSpend).toFixed(2))
  },
  ['adfuel-balance'],
  { revalidate: 300 } // 5-minute server cache per locationId
)
...
export async function GET(request: NextRequest) {
  ...
  if (!locationId) {
    return NextResponse.json({ error: 'Missing locationId' }, { status: 400, headers: CORS_HEADERS })
  }

  const balance = await getAdFuelBalance(locationId)
  if (balance === null) {
    return NextResponse.json({ error: 'locationId not found' }, { status: 404, headers: CORS_HEADERS })
  }
