...
//   1. For any open Stripe invoice with a non-canceled payment intent, auto-create
//      a pending ledger entry if one doesn't exist yet. No date filter — subscription
//      update invoices carry the original subscription created date, so a date filter
//      would silently exclude them. Product data is expanded so isAdFuelLine() can
//      check product names for subscription line items with null descriptions.
    ...
    if (stripe) {
      ...
      for (const inv of invoices.data) {
        ...
        if ((inv.lines as unknown as { has_more?: boolean }).has_more) {
          console.warn(`[pending-ach] Invoice ${inv.id} has paginated line items — ad fuel total may be partial`)
        }
