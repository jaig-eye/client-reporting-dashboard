...
import { getAuthors }        from '@/lib/connectors/wordpress'

/**
 * GET /api/admin/wordpress/authors?connection_id=<uuid>
 *
 * Returns the list of WordPress users (administrators + editors) for the
 * given WordPress client_connection. Used to populate the author dropdown
 * in the ContentPostEditor.
 */
export async function GET(request: NextRequest) {
  ...
  const authors = await getAuthors(siteUrl, { username, app_password: appPassword })
