...
// Hourly cron — detects pending ACH invoices and resolves them.
//
//   Detect:  open Stripe invoices (last 3 days) with non-failed payment intent
//            → insert into ad_fuel_ach_pending if not already tracked
//   Paid     → insert confirmed row into ad_fuel_ledger + delete from ad_fuel_ach_pending
//   Void/uncollectible or payment definitively failed → delete from ad_fuel_ach_pending
//   Still open / no PI (credit transfer) → leave alone
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    for (const inv of invoices.data) {
      ...
      console.log(`[ad-fuel-ach-clear] detected pending ACH invoice=${inv.id} client=${clientId} amount=${totalAf}`)
      detected++
    }
  } catch (err) {
    ...
    try {
      ...
      if (inv.status === 'paid') {
        ...
        const { error: insertErr } = await db.from('ad_fuel_ledger').insert({
          ...
          date_of_payment: clearedDate,
          invoice_date:    entry.invoice_date,
          amount_af:       entry.amount_af,
          invoice_id:      entry.invoice_id,
          type:            'ACH',
          created_by:      'auto-ach',
          note:            (entry.note ?? '').replace('pending', 'cleared'),
        })
        if (insertErr) {
        ...
        console.log(`[ad-fuel-ach-clear] cleared entry ${entry.id} client=${entry.client_id} on ${clearedDate}`)
        cleared++
        ...
        // No PI = ACH credit transfer, customer is pushing money — wait for paid
        if (!pi) continue
