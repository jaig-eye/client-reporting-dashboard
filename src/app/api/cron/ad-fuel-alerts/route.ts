...
export async function GET(request: NextRequest) {
  ...
  type ClientRow = { id: string; name: string; ad_fuel_cut: number | null; historic_bill_day: number | null; discord_channel_id: string | null; ad_fuel_alert_threshold: number | null; last_fuel_alert_at: string | null }

  // Round 2: remaining queries using correct cutoffDate
  const [clientsRes, ledgerRes, gSpendRes, mSpendRes] = await Promise.all([
    db.from('clients').select('id, name, ad_fuel_cut, historic_bill_day, discord_channel_id, ad_fuel_alert_threshold, last_fuel_alert_at').not('discord_channel_id', 'is', null),
