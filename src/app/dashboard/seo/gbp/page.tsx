// ─────────────────────────────────────────────────────────────────────────────
// GBP (Google Business Profile) Page — /dashboard/seo/gbp
...
export default async function GBPPage({
  ...
  const totals = gbpRows.reduce(
    (acc, r) => ({
      ...
      reviews_count:    0,  // use max per location, not sum
  ...
  for (const r of gbpRows) {
    if (!latestByLocation.has(r.location_id)) {
      ...
      // gbpRows is ordered by date asc; last one per location wins
      latestByLocation.set(r.location_id, { rating: r.reviews_avg_rating ?? 0, count: r.reviews_count ?? 0 })
    }
  }
  ...
  // Daily trend for chart: total views + website_clicks per day (sum across locations)
  ...
  const locations = Array.from(new Set(gbpRows.map(r => r.location_id)))
    .map(id => {
      const rows = gbpRows.filter(r => r.location_id === id)
      const name = rows[rows.length - 1]?.location_name ?? id
      const loc_totals = rows.reduce(
        (acc, r) => ({
    ...
    <header className="sticky top-0 z-10 border-b" style={{ background: 'var(--bg-surface)', borderColor: 'var(--border)', boxShadow: '0 1px 3px rgba(0,0,0,0.06)' }}>
      <div className="max-w-7xl mx-auto px-6 py-3 flex items-center justify-between gap-4">
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-sm" style={{ color: 'var(--text-primary)' }}>SEO — Google Business Profile</span>
