...
-- ─── 1. Silo keyword map ─────────────────────────────────────────────────────
-- Stores the keyword universe for each silo: top-level, secondary, and supporting.

CREATE TABLE content_silo_keywords (
...
-- Pre-planned pages in a silo before generation. Knocked off as content is created.

CREATE TABLE content_silo_pages (
  id                   UUID        DEFAULT gen_random_uuid() PRIMARY KEY,
  client_id            UUID        NOT NULL REFERENCES clients(id) ON DELETE CASCADE,
  silo_id              UUID        NOT NULL REFERENCES content_silos(id) ON DELETE CASCADE,
...
-- Persistent structured brief for a page. Drives generation and scoring.

CREATE TABLE content_optimization_briefs (
  id                                UUID        DEFAULT gen_random_uuid() PRIMARY KEY,
  client_id                         UUID        NOT NULL REFERENCES clients(id) ON DELETE CASCADE,
...
-- Scored audit snapshot for a page at a point in time.

CREATE TABLE content_optimization_audits (
  id                      UUID        DEFAULT gen_random_uuid() PRIMARY KEY,
  client_id               UUID        NOT NULL REFERENCES clients(id) ON DELETE CASCADE,
  silo_id                 UUID        REFERENCES content_silos(id) ON DELETE SET NULL,
  silo_page_id            UUID        REFERENCES content_silo_pages(id) ON DELETE SET NULL,
...
-- Tracks recommended, inserted, and ignored internal links within a silo cluster.

CREATE TABLE content_silo_internal_links (
  id                    UUID        DEFAULT gen_random_uuid() PRIMARY KEY,
  client_id             UUID        NOT NULL REFERENCES clients(id) ON DELETE CASCADE,
  silo_id               UUID        NOT NULL REFERENCES content_silos(id) ON DELETE CASCADE,
